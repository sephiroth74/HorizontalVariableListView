...
public class HListView extends AbsHListView {
	...
	/**
	 * When arrow scrolling, need a certain amount of pixels to preview next items. This is usually the fading edge, but if that is small enough, we
	...
	private final ArrayList<FixedViewInfo>	mHeaderViewInfos		= new ArrayList<HListView.FixedViewInfo>();
	private final ArrayList<FixedViewInfo>	mFooterViewInfos		= new ArrayList<HListView.FixedViewInfo>();
	 ...
	 * <p>
	 * NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor with one that will also account for header and
	 * footer views.
	 * 
	 * @param v
	 *            The view to add.
	...
	/**
	 * Add a fixed view to appear at the bottom of the list. If addFooterView is called more than once, the views will appear in the order they were
	 * added. Views added using this call can take focus if they want.
	 * <p>
	 * NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor with one that will also account for header and
	 * footer views.
	 ...
	 * 
	 * The adapter passed to this method may be wrapped by a {@link WrapperListAdapter}, depending on the ListView features currently in use. For
	 * instance, adding headers and/or footers will cause the adapter to be wrapped.
	 * 
	 * @param adapter
	 *            The ListAdapter which is responsible for maintaining the data backing this list and for producing a view to represent an item in
	 *            that data set.
	 * 
	 * @see #getAdapter()
	 */
	...
	/**
	 * Fills the grid based on positioning the new selection at a specific location. The selection may be moved so that it does not intersect the
	...
	/**
	 * Fills the list based on positioning the new selection relative to the old selection. The new selection will be placed at, above, or below the
	 * location of the new selection depending on how the selection is moving. The selection will then be pinned to the visible part of the screen,
	 * excluding the edges that are faded. The list is then filled upwards and downwards from there.
	 * 
	 * @param oldSel
	...
	/**
	 * Measures the height of the given range of children (inclusive) and returns the height with this ListView's padding and divider heights
	 ...
	 * @param endPosition
	 *            The (inclusive) position of the last child to be shown. Specify {@link #NO_POSITION} if the last child should be the last available
	 *            child from the adapter.
	 * @param maxWidth
	 ...
	 *            In general, whether the returned height should only contain entire children. This is more powerful--it is the first inclusive
	 *            position at which partial children will not be allowed. Example: it looks nice to have at least 3 completely visible children, and
	 *            in portrait this will most likely fit; but in landscape there could be times when even 2 children can not be completely shown, so a
	 *            value of 2 (remember, inclusive) would be good (assuming startPosition is 0).
	...
	final int[] measureWithLargeChildren(int heightMeasureSpec, int startPosition, int endPosition, final int maxWidth, final int maxHeight,
	...
	/**
	 * Check if we have dragged the right of the list too wide (we have pushed the left element off the left of the screen when we did not need to).
	...
	/**
	 * Check if we have dragged the right of the list too low (we have pushed the right element off the right of the screen when we did not need to).
	...
	protected void layoutChildren() {
		...
		try {
			...
			/*
			 * // Remember which child, if any, had accessibility focus. final ViewRootImpl viewRootImpl = getViewRootImpl(); if ( viewRootImpl !=
			 * null ) { final View accessFocusedView = viewRootImpl.getAccessibilityFocusedHost(); if ( accessFocusedView != null ) { final View
			 * accessFocusedChild = findAccessibilityFocusedChild( accessFocusedView ); if ( accessFocusedChild != null ) { if ( !dataChanged ||
			 * isDirectChildHeaderOrFooter( accessFocusedChild ) ) { // If the views won't be changing, try to maintain // focus on the current view
			 * host and (if // applicable) its virtual view. accessibilityFocusLayoutRestoreView = accessFocusedView;
			 * accessibilityFocusLayoutRestoreNode = viewRootImpl .getAccessibilityFocusedVirtualView(); } else { // Otherwise, try to maintain focus
			 * at the same // position. accessibilityFocusPosition = getPositionForView( accessFocusedChild ); } } } }
			 */
	...
	/**
	 * Obtain the view and add it to our list of children. The view can be made fresh, converted from an unused view, or used as is if it was in the
	 * recycle bin.
	 * 
	 * @param position
	...
	/**
	 * Sets the currently selected item. If in touch mode, the item will not be selected but it will still be positioned appropriately. If the
	 * specified selection position is less than 0, then the item at position 0 will be selected.
	 * 
	 * @param position
	...
	/**
	 * Sets the selected item and positions the selection x pixels from the left edge of the ListView. (If in touch mode, the item will not be
	 * selected but it will still be positioned appropriately.)
	 * 
	 * @param position
	 *            Index (starting at 0) of the data item to be selected.
	 * @param x
	 ...
	 * @return The next selectable position starting at position and then searching either up or down. Returns {@link #INVALID_POSITION} if nothing
	 *         can be found.
	 */
	@Override
	...
	/**
	 * Go to the last or first item if possible (not worrying about panning across or navigating within the internal focus of the currently selected
	 * item.)
	 * 
	 * @param direction
	 *            either {@link View#FOCUS_UP} or {@link View#FOCUS_DOWN}
	...
	/**
	 * To avoid horizontal focus searches changing the selected item, we manually focus search within the selected item (as applicable), and prevent
	 * focus from jumping to something within another item.
	 * 
	 * @param direction
	...
	/**
	 * When selection changes, it is possible that the previously selected or the next selected item will change its size. If so, we need to offset
	 * some folks, and re-layout the items as appropriate.
	 * 
	 * @param selectedView
	 *            The currently selected view (before changing selection). should be <code>null</code> if there was no previous selection.
	 * @param direction
	 *            Either {@link android.view.View#FOCUS_UP} or {@link android.view.View#FOCUS_DOWN}.
	 * @param newSelectedPosition
	 *            The position of the next selection.
	 * @param newFocusAssigned
	 *            whether new focus was assigned. This matters because when something has focus, we don't want to show selection (ugh).
	 */
	...
	/**
	 * Determine how much we need to scroll in order to get the next selected view visible, with a fading edge showing below as applicable. The amount
	 ...
	 * @param direction
	 *            either {@link android.view.View#FOCUS_UP} or {@link android.view.View#FOCUS_DOWN}.
	 * @return The position of the next selectable position of the views that are currently visible, taking into account the fact that there might be
	 *         no selection. Returns {@link #INVALID_POSITION} if there is no selectable view on screen in the given direction.
	 */
	...
	/**
	 * Do an arrow scroll based on focus searching. If a new view is given focus, return the selection delta and amount to scroll via an
	 ...
	 * @param childIndex
	 *            The index of child (of the View) above the divider. This will be -1 if there is no child above the divider to be drawn.
	 */
	...
	/**
	 * Sets the drawable that will be drawn between each item in the list. If the drawable does not have an intrinsic height, you should also call
	...
	/**
	 * Sets the height of the divider that will be drawn between each item in the list. Calling this will override the intrinsic height as set by
	...
	/**
	 * Sets the drawable that will be drawn below all other list content. This area can become visible when the user overscrolls the list, or when the
	 ...
	 * (non-Javadoc)
	 * 
	 * Children specified in XML are assumed to be header views. After we have parsed them move them out of the children list and into mHeaderViews.
	 */
	@Override
	 ...
	 * super.findViewByPredicateTraversal( predicate, childToSkip ); if ( v == null ) { v = findViewByPredicateInHeadersOrFooters( mHeaderViewInfos,
	 * predicate, childToSkip ); if ( v != null ) { return v; }
	 * 
	 * v = findViewByPredicateInHeadersOrFooters( mFooterViewInfos, predicate, childToSkip ); if ( v != null ) { return v; } } return v; }
	 */

	/*
	 * View findViewByPredicateInHeadersOrFooters( ArrayList<FixedViewInfo> where, Predicate<View> predicate, View childToSkip ) { if ( where != null
	...
	/**
	 * Returns the set of checked items ids. The result is only valid if the choice mode has not been set to {@link #CHOICE_MODE_NONE}.
	 * 
