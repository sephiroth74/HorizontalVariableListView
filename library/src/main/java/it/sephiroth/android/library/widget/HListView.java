...
public class HListView extends AbsHListView {
     ...
     * depending on the ListView features currently
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.UNSPECIFIED) {
            ...
            int[] result =
                measureWithLargeChildren(heightMeasureSpec, mMeasureWithChild, mMeasureWithChild, widthSize, heightSize, -1);
            heightSize = result[1];
    ...
    /**
     * Measures the height of the given range of children (inclusive) and returns the height with this ListView's padding and
     * divider
     ...
     * @param endPosition                  The (inclusive) position of the last child to be shown. Specify {@link #NO_POSITION}
     *                                     if the last child should be the
     *                                     last available child from the adapter.
     * @param maxWidth                     The maximum height that will be returned (if all the children don't fit in this value,
     *                                     this value will be returned).
     * @param disallowPartialChildPosition In general, whether the returned height should only contain entire children. This is
     *                                     more powerful--it is the first
     *                                     inclusive position at which partial children will not be allowed. Example: it looks
     *                                     nice to have at least 3
     *                                     completely visible children, and in portrait this will most likely fit; but in
     *                                     landscape there could be times when
     *                                     even 2 children can not be completely shown, so a value of 2 (remember,
     *                                     inclusive) would be good (assuming
     *                                     startPosition is 0).
     * @return The height of this ListView with the given children.
     */
    final int measureWidthOfChildren(
        int heightMeasureSpec, int startPosition, int endPosition,
    ...
    final int[] measureWithLargeChildren(
        int heightMeasureSpec, int startPosition, int endPosition, final int maxWidth, final int maxHeight,
    ...
    /**
     * Check if we have dragged the right of the list too wide (we have pushed the left element off the left of the screen when
     * we did
    ...
    private void setupChild(
        View child, int position, int x, boolean flowDown, int childrenTop, boolean selected, boolean recycled) {
        ...
        if (needToMeasure) {
            int childHeightSpec =
                ViewGroup.getChildMeasureSpec(mHeightMeasureSpec, mListPadding.top + mListPadding.bottom, p.height);
            int lpWidth = p.width;
            int childWidthSpec;
            if (lpWidth > 0) {
                childWidthSpec = MeasureSpec.makeMeasureSpec(lpWidth, MeasureSpec.EXACTLY);
    ...
    private boolean commonKey(int keyCode, int count, KeyEvent event) {
        if (mAdapter == null || !mAttachedToWindow) {
            return false;
        }
     ...
     * @param newFocusAssigned    whether new focus was assigned. This matters because when something has focus,
     *                            we don't want to show selection
     *                            (ugh).
     */
    ...
    private void measureItem(View child) {
        ...
        int childHeightSpec = ViewGroup.getChildMeasureSpec(
            mHeightMeasureSpec, mListPadding.top + mListPadding.bottom, p
                .height);
        int lpWidth = p.width;
        int childWidthSpec;
        if (lpWidth > 0) {
     ...
     *                             selectable position
     * @return The amount to scroll. Note: this is always positive! Direction needs to be taken into account when actually
     * scrolling.
     */
    ...
    /**
     * Do an arrow scroll based on focus searching. If a new view is given focus, return the selection delta and amount to scroll
     * via
            ...
            if (!mStackFromRight) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if ((headerDividers || !isHeader) && (footerDividers || !isFooter)) {
                        ...
                        if (drawDividers && (right < listRight)
     ...
     * @param childIndex The index of child (of the View) above the divider. This will be -1 if there is no child above the
     *                   divider to be
     *                   drawn.
     */
	 ...
	 * 
	 * v = findViewByPredicateInHeadersOrFooters( mFooterViewInfos, predicate, childToSkip ); if ( v != null ) { return v; } }
	 * return
	 * v; }
	 */
    ...
    int getWidthForPosition(int position) {
        final int width = super.getWidthForPosition(position);
        if (shouldAdjustWidthForDivider(position)) {
            return width + mDividerWidth;
        }
        return width;
    }
